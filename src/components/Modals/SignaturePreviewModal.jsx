...
export default function SignaturePreviewModal({ isOpen, onClose, image, onBack, onSaveFinal }) {
  ...
  const downloadSignature = () => {
    let width = 600,
      height = 200;
    if (resolution === "300x100") {
      width = 300;
      height = 100;
    } else if (resolution === "600x200") {
      width = 600;
      height = 200;
    } else if (resolution === "custom") {
      width = Number(customWidth || 600);
      height = Number(customHeight || 200);
    }
    ...
    >
      ...
      >
        <div className="flex items-center mb-4">
          ...
          <button
            onClick={onBack}
            className="px-4 py-1 ml-auto bg-gray-200 rounded-md hover:bg-red-600 hover:text-white"
          >
            Back
          </button>
        </div>
        ...
        <div className="w-full h-48 border border-gray-300 rounded-md mb-4 flex items-center justify-center overflow-hidden">
          {image ? (
            ...
            <span className="text-gray-400">No signature found</span>
          )}
        </div>
        ...
        <div className="flex justify-between mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-md w-1/2 mr-2 ${
              format === "png" ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"
            }`}
          >
            PNG
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md w-1/2 ml-2 ${
              format === "jpg" ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"
            }`}
          >
            JPG
          </button>
